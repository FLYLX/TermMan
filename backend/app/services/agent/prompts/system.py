...
ROBOT_MESSAGING_FALLBACK_PROMPT = (
    ...
    "chat. "
    "judge whether QQ should receive a message from the current message plus "
    "recent QQ context. If the message is `reply_to_bot`, mentions the bot, "
    "or has `trigger=active_chat_window`, treat it as a candidate continuation "
    "of the bot conversation. Call `mcp_robot_send_message` when the sender is "
    "addressing you, continuing a conversation with you, challenging or "
    "correcting your prior behavior, asking for a useful response, or when a "
    "severe alert explicitly needs QQ notification. Do not reply to ordinary "
    "group chatter with no contextual link to the bot, messages directed at "
    "someone else, or messages that do not need a bot response. Never "
