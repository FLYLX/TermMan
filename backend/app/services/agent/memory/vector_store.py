...
class VectorStoreService:
    ...
    def _initialize_memory_collection(self) -> None:
        ...
        supports_migration = all(
            ...
            for method in ("list_collections", "delete_collection", "get_or_create_collection")
        )
    ...
    def _migrate_memory_collection(
        ...
        *,
        ...
        dimension = len(embeddings[0]) if embeddings else self._embedding_service.dimension()
        ...
        try:
            ...
            source_metadata["superseded_by"] = str(
    ...
    def _delete_lexical_memories(
        ...
        *,
        ...
        value = str(memory_id if memory_id is not None else item_id)
        with self._lexical_index_lock:
            try:
        ...
        if self._collection is not None:
            try:
                ...
                for index, memory_id in enumerate(result.get("ids") or []):
                    records.append({
                        "id": str(memory_id),
                        "content": str((result.get("documents") or [])[index] or ""),
                    ...
                    for record in records:
                        ...
                        connection.execute(
                            ...
                            (
                                ...
                                json.dumps(metadata, ensure_ascii=False, default=str),
                                self._lexical_token_text(str(record.get("content") or "")),
                            ),
                        )
            except Exception as exc:
                logger.warning("[VectorStore] Failed to rebuild SQLite FTS5 index: %s", exc)
    ...
    def _search_lexical_index(
        ...
        *,
        ...
        params: list[Any] = [match_query, str(item_id)]
        if memory_type:
            sql += " AND memory_type = ?"
    ...
    def add_memory(
        ...
        if run_maintenance:
            self._maybe_compact_expired_memories(item_id)
        return saved_id
    ...
    def search_memories(
        ...
        *,
        ...
        self._ensure_initialized()
        self.maintain_memories(item_id)
    ...
    def delete_item_memories(self, item_id: str):
        ...
        if len(kept) != len(fallback_memories):
            self._write_fallback_memories(kept)
        self._delete_lexical_memories(item_id=item_id)
        with self._maintenance_lock:
            self._maintenance_items.discard(str(item_id))
            self._maintenance_last_checked.pop(str(item_id), None)
        ...
        *,
        ...
        for memory in sorted(
            ...
            if len(prefix) + len(candidate) <= EXPIRED_MEMORY_SUMMARY_MAX_CHARS:
                ...
                continue
            remaining = EXPIRED_MEMORY_SUMMARY_MAX_CHARS - len(prefix) - len("；".join(parts))
            if remaining > 24:
                parts.append(content[: max(1, remaining - 2)].rstrip() + "…")
            break
        return prefix + "；".join(parts)

    def _enter_memory_maintenance(self, item_id: str) -> bool:
        with self._maintenance_lock:
            if item_id in self._maintenance_items:
                return False
            self._maintenance_items.add(item_id)
            return True

    def _leave_memory_maintenance(self, item_id: str) -> None:
        with self._maintenance_lock:
            self._maintenance_items.discard(item_id)

    def _maybe_compact_expired_memories(self, item_id: str) -> None:
        try:
            self.maintain_memories(item_id, interval_seconds=0)
        except Exception as exc:
            logger.warning(
        ...
        *,
        ...
        try:
            ...
            for group in groups.values():
                if len(group) < 2:
                    continue
                source_type = str((group[0].get("metadata") or {}).get("memory_type") or "fact")
    ...
    def expire_old_memories(self, item_id: str) -> int:
        ...
        expired_ids = {
            ...
            for memory in self.get_all_memories(item_id)
            if memory.get("id")
