...
for check in range(5):
    print(f"\n{'='*50}")
    print(f"Check {check+1}/5 (waiting 30s...)")
    time.sleep(30)
    ...
    for w in wf.get("workflows", []):
        print(f"WF: status={w.get('status')} obj={w.get('objective','')[:60]}")
    ...
    java_count = result.stdout.strip()
    print(f"Java processes: {java_count}")