import { Plus, Trash2, GripVertical, Code, List, ChevronDown, ChevronRight } from "lucide-react"
...
const ACTION_TYPE_LABELS: Record<string, { label: string; description: string; variant: "destructive" | "default" | "secondary" | "outline" }> = {
  block: { label: "拦截", description: "匹配后拦截，不输出", variant: "destructive" },
  ignore: { label: "忽略", description: "匹配后忽略，标记为噪音", variant: "secondary" },
  log: { label: "输出", description: "匹配后输出，标记为需关注", variant: "default" },
  replace: { label: "替换", description: "匹配后替换内容", variant: "outline" },
}

export function FilterRuleEditor({ value, onChange, defaultRules }: FilterRuleEditorProps) {
  ...
  useEffect(() => {
    if (mode === "ui") {
      const newFilters = Object.entries(value || {}).map(([name, rule]) => ({ ...rule, name }))
  ...
  const updatePattern = (filterIndex: number, patternIndex: number, value: string) => {
  ...
  const removePattern = (filterIndex: number, patternIndex: number) => {
    const newPatterns = filters[filterIndex].regex_patterns.filter((_, i) => i !== patternIndex)
    updateFilter(filterIndex, { regex_patterns: newPatterns })
  }

  const updateReplaceRule = (filterIndex: number, pattern: string, replacement: string) => {
      ...
      if (text.trim()) {
        ...
        for (const [name, config] of Object.entries(parsed)) {
          if (typeof config === "object" && config !== null) {
            ...
            newFilters.push({
              name,
              regex_patterns: Array.isArray(cfg.regex_patterns) ? cfg.regex_patterns as string[] : [],
              action_type: (cfg.action_type as FilterRule["action_type"]) || "ignore",
  ...
  const loadDefaultRules = () => {
    const newFilters = Object.entries(defaultRules).map(([name, rule]) => ({ ...rule, name }))
    setFilters(newFilters)
              ...
              >
                <CollapsibleTrigger asChild>
                  <div className={`flex items-center gap-2 p-3 cursor-pointer transition-colors ${
                    filter.action_type === "block" ? "bg-red-50 hover:bg-red-100 dark:bg-red-950/30 dark:hover:bg-red-950/50" :
                    filter.action_type === "ignore" ? "bg-gray-50 hover:bg-gray-100 dark:bg-gray-950/30 dark:hover:bg-gray-950/50" :
                    filter.action_type === "log" ? "bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/30 dark:hover:bg-blue-950/50" :
                    ...
                    <div className="flex items-center gap-1 flex-1">
                      ...
                      <span className={`ml-2 px-2 py-0.5 text-xs rounded-full ${
                        filter.action_type === "block" ? "bg-red-500 text-white" :
                        filter.action_type === "ignore" ? "bg-gray-500 text-white" :
                        filter.action_type === "log" ? "bg-blue-500 text-white" :
                  ...
                  <div className="p-4 space-y-4 border-t">
                    <div className="flex items-end gap-4">
                      <div className="space-y-1">
                        ...
                        <Input
                          value={filter.name}
                          onChange={(e) => updateFilter(filterIndex, { name: e.target.value })}
                          placeholder="过滤器名称"
                          className="w-48"
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <div className="flex gap-1">
                          {Object.entries(ACTION_TYPE_LABELS).map(([type, info]) => (
                            <button
                              key={type}
                              type="button"
                              onClick={() => updateFilter(filterIndex, { action_type: type as FilterRule["action_type"] })}
                    ...
                    <div className="space-y-2">
                      ...
                      {filter.regex_patterns.map((pattern, patternIndex) => (
                        <div key={patternIndex} className="flex gap-2">
                          <Input
                            value={pattern}
                            onChange={(e) => updatePattern(filterIndex, patternIndex, e.target.value)}
                            placeholder="正则表达式"
                          ...
                          {filter.regex_patterns.length > 1 && (
                            <Button
                              size="icon"
                              variant="ghost"
                              onClick={() => removePattern(filterIndex, patternIndex)}
                              className="text-destructive hover:text-destructive shrink-0"
                            >
                    ...
                    {filter.action_type === "replace" && (
                      <div className="space-y-2">
                        ...
                        {filter.regex_patterns.filter(p => p).map((pattern) => (
                          <div key={pattern} className="flex gap-2 items-center">
                            <Input
                              value={pattern}
                              disabled
                            ...
                            <Input
                              value={filter.action?.replace_rules?.[pattern] || ""}
                              onChange={(e) => updateReplaceRule(filterIndex, pattern, e.target.value)}
                              placeholder="替换为"
                              className="flex-1"
                            />
                          </div>
