...
本文档记录 TermMan 当前采用的 Agent 记忆方案。目标不是把所有历史都塞给模型，而是让 Agent 在 QQ、后台聊天、终端事件和工具调用场景里稳定记住真正可复用的信息，同时降低旧记忆污染和幻觉。

## 选型

当前方案采用 **Mem0-like 轻量长期记忆 + MemoryOS 分层思想**。

不直接采用 MemOS、MemTree、Zep 这类重方案，原因是 TermMan 的核心问题不是大规模关系推理，而是：
