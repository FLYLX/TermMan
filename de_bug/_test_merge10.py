"""Send 10 rapid messages: first processes, other 9 should merge into ONE batch."""
...
async def main():
    ...
    async with websockets.connect(WS_URL, additional_headers=HEADERS,
        ...
        t0 = time.time()
        for i in range(1, 11):
            await send_qq(ws, f"第{i}条：{i}+{i}等于几")
            log(f"sent msg {i}")
            await asyncio.sleep(0.4)

        log("--- all 10 sent, watching replies ---")
        ...
        for ts, text in replies:
            log(f"  +{ts - t0:5.1f}s  {text[:120]}")
        if len(replies) <= 2:
            log("PASS: 9 follow-ups merged (<=2 replies total)")
        else:
            log(f"FAIL: {len(replies)} separate replies (no merge)")
