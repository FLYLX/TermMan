...
# ---------- 1. hooks.py: veto aggregation ----------
p = r"backend\app\services\agent\integrations\hooks.py"
t, crlf = load(p)
old = """def integration_memory_scope_rank(agent: Agent, memory: dict[str, Any]) -> int:
    best = 0
    for integration in get_agent_integrations():
        ...
        if rank > best:
            best = rank
    return best"""
new = """def integration_memory_scope_rank(agent: Agent, memory: dict[str, Any]) -> int:
    best = 0
    for integration in get_agent_integrations():
        rank = integration.memory_scope_rank(agent, memory)
        if rank < 0:
        ...
        if rank > best:
            best = rank
    return best"""
t = replace_once(t, old, new, "hooks veto aggregation")
save(p, t, crlf)
print("hooks.py OK")

# ---------- 2-7. test_robot_service.py ----------
...
def _process_captured_chat_job(monkeypatch) -> list[str]:"""
