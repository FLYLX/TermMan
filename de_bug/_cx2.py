...
td = r3.json()
print(f"Tokens: total={td['total_tokens']}, turns={td['total_turns']}, prompt={td['total_prompt_tokens']}, comp={td['total_completion_tokens']}")
...
for j in jobs.get("jobs", []):
    print(f"  [{j.get('status','running')}] {j.get('job_id','')} elapsed={j.get('elapsed_seconds',0):.0f}s cmd={str(j.get('command',''))[:70]}")
...
r2 = requests.get(f"{BASE}/api/v1/memory/{ITEM}/session?limit=6&offset=0", headers=h, timeout=10)
msgs = r2.json().get("messages", [])
print(f"\nLast messages ({len(msgs)}):")
for m in msgs:
    mtype = m.get("type", "")
